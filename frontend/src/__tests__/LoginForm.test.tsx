...
describe('Tela de Login (LoginPage Component & BDD Scenarios)', () => {
  ...
  it('Cenário 2: Exibe validações visuais ao tentar submeter formulário em branco ou com formato de email inválido', async () => {
    renderLoginPage();
    ...
    // Testa email sem @
    await userEvent.type(screen.getByLabelText(/endereço de email/i), 'emailinvalido');
    fireEvent.click(screen.getByRole('button', { name: /entrar no sistema/i }));

    expect(await screen.findByText('Por favor, insira um formato de email válido.')).toBeInTheDocument();
